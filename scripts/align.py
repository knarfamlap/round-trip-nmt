...
def gather_null_aligns(sim_matrix: np.ndarray,
                       inter_matrix: np.ndarray) -> List[float]:
...
def apply_percentile_null_aligns(sim_matrix: np.ndarray,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog=
        "example: python3 main.py path/to/L1/text path/to/L2/text [options]")
    parser.add_argument(
        "L1_path",
        type=str,
    ...
    parser.add_argument("--token-type",
                        type=str,
                        choices=["bpe", "word"],
                        default="bpe")
    parser.add_argument(
        ...
        default="mai",
        help=
        "m: Max Weight Matching (mwmf), a: argmax (inter), i: itermax, f: forward (fwd), r: reverse (rev)"
    )
    parser.add_argument("--num-test-sents",
                        type=int,
                        default=None,
                        help="None means all sentences")
    ...
    for path in original_paths:
        ...
        corpus = [
            regex.sub("\\p{C}+", "", regex.sub("\\p{Separator}+", " ",
                                               l)).strip() for l in corpus
        ]
    ...
    for sent_id in range(len(words_tokens)):
        sent_pair = [[bpe for w in sent for bpe in w]
                     for sent in words_tokens[sent_id]]
        b2w_map_pair = [[i for i, w in enumerate(sent) for bpe in w]
                        for sent in words_tokens[sent_id]]
    ...
    out_f = {
        ext: open('{}.{}'.format(args.output, ext), 'w')
    ...
    if args.null_align < 1.0:
        ...
        for sent_id in range(len(original_corpora[0])):
            ...
            vectors = embed_loader.get_embed_list(
                sent_pair).cpu().detach().numpy()
            vectors = [
                vectors[i][:len(sentences_bpe_lists[sent_id][i])]
                for i in [0, 1]
            ]

            if convert_to_words:
                vectors = SentenceAligner.average_embeds_over_words(
                    vectors, words_tokens[sent_id])
            ...
            methods_matrix = {}
            methods_matrix["forward"], methods_matrix[
                "backward"] = SentenceAligner.get_alignment_matrix(sim)
            methods_matrix["inter"] = methods_matrix[
                "forward"] * methods_matrix["backward"]
            if "mwmf" in matching_methods:
                methods_matrix["mwmf"] = SentenceAligner.get_max_weight_match(
                    sim)
        ...
        null_thresh = {
            m: sorted(entropies[m])[int(args.null_align * len(entropies[m]))]
            for m in entropies
        }

    ds = [(idx, original_corpora[0][idx], original_corpora[1][idx])
    ...
    for batch_id, batch_sentences in enumerate(data_loader):
        ...
        if not convert_to_words:
            ...
            btach_sim = torch.bmm(batch_vectors_src,
                                  torch.transpose(batch_vectors_trg, 1, 2))
        ...
        for in_batch_id, sent_id in enumerate(batch_sentences[0].numpy()):
            ...
            if not convert_to_words:
                sim = btach_sim[
                    in_batch_id, :len(sent_pair[0]), :len(sent_pair[1])]
            else:
                vectors = SentenceAligner.average_embeds_over_words(
                    vectors, words_tokens[sent_id])
                sim = SentenceAligner.get_similarity(vectors[0], vectors[1])
            ...
            if args.null_align < 1.0:
                mask_nulls = {
                    mmethod:
                    apply_percentile_null_aligns(sim, null_thresh[mmethod])
                    for mmethod in matching_methods
                }

            all_mats["fwd"], all_mats[
                "rev"] = SentenceAligner.get_alignment_matrix(sim)
            ...
            if args.null_align < 1.0:
                if "inter" in matching_methods:
                    all_mats["inter"] = np.multiply(all_mats["inter"],
                                                    mask_nulls["inter"])
                if "mwmf" in matching_methods:
                    all_mats["mwmf"] = np.multiply(all_mats["mwmf"],
                                                   mask_nulls["mwmf"])
                if "itermax" in matching_methods:
                    all_mats["itermax"] = np.multiply(all_mats["itermax"],
                                                      mask_nulls["itermax"])
            ...
            for ext in out_f:
                if convert_to_words:
                    out_f[ext].write(
                        str(sent_id) + "\t" +
                        ' '.join(sorted(raw_aligns[ext])) + "\n")
                else:
                    out_f[ext].write(
                        str(sent_id) + "\t" +
                        ' '.join(sorted(b2w_aligns[ext])) + "\n")
            if args.log:
                out_log.write(
                    str(sent_id) + "\t" + ' '.join(sorted(log_aligns)) + "\n")
